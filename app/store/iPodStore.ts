  ...
  persist(
    (set, get) => ({
      ...
      navigateTo: (screen) => set((state) => ({
        navigationStack: [...state.navigationStack, { ...state.currentScreen, selectedIndex: state.selectedIndex }],
      ...
      incrementSelectedIndex: (max) => set((state) => ({
        selectedIndex: Math.min(state.selectedIndex + 1, max - 1),
        lastActivityTime: Date.now(),
        backlightOn: true,
      })),
      
      decrementSelectedIndex: () => set((state) => ({
      ...
      previousTrack: () => set((state) => {
        ...
        if (state.currentTime > 3) {
          return { currentTime: 0, lastActivityTime: Date.now(), backlightOn: true };
        }
        
        let prevIndex = state.queueIndex - 1;
        if (prevIndex < 0) {
          prevIndex = state.settings.repeat === 'all' ? state.queue.length - 1 : 0;
        }
      ...
      scrub: (delta) => set((state) => {
        if (!state.currentSong || !state.isScrubbing) return state;
        const newTime = Math.max(0, Math.min(state.currentSong.duration, state.currentTime + delta));
        return { 
          currentTime: newTime,
          lastActivityTime: Date.now(),
          backlightOn: true,
      ...
      // Rating actions
      setRating: (songId, rating) => set((state) => ({
      ...
      toggleSleep: () => {
        const state = get();
        if (state.isSleeping) {
          set({ isSleeping: false, backlightOn: true, lastActivityTime: Date.now() });
        } else {
      ...
      // Photo actions
      setCurrentPhoto: (albumId, index) => set({

...
export default function ClickWheel({
  ...
  const { settings, isScrubbing, resetBacklight } = useIPodStore();
  
  // Long press handling
  const [centerPressTimer, setCenterPressTimer] = useState<NodeJS.Timeout | null>(null);
  const [playPressTimer, setPlayPressTimer] = useState<NodeJS.Timeout | null>(null);
  ...
  }, [onScroll, onSelect, onMenu, onPlayPause, onNext, onPrevious, playClick, disabled, onPlayPauseLongPress, resetBacklight]);
    ...
    >
      ...
      >
        <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
          ...
          <path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z" transform="translate(12, 0)" />
        </svg>
      </button>
